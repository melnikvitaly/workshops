#pragma once
#include <esp_adc/adc_oneshot.h>
#include <esp_adc/adc_cali.h>
#include <esp_adc/adc_cali_scheme.h>
#include <esp_log.h>
#include <esp_check.h>

class ADC
{
    adc_unit_t _unit;
    adc_channel_t _channel;
    adc_atten_t _atten;
    adc_bitwidth_t _bitwidth;
    int _maxMv;
    int _maxRaw;
    adc_oneshot_unit_handle_t _handle = nullptr;
    adc_cali_handle_t _cali = nullptr;

    static int rawMax(adc_bitwidth_t w)
    {
        int bits = (w == ADC_BITWIDTH_DEFAULT) ? 12 : (int)w;
        return (1 << bits) - 1;
    }

public:
    ADC(adc_unit_t unit, adc_channel_t channel,
        adc_atten_t atten,
        int maxMv,
        adc_bitwidth_t bitwidth)
        : _unit(unit), _channel(channel), _atten(atten),
          _bitwidth(bitwidth), _maxMv(maxMv), _maxRaw(rawMax(bitwidth)) {}

    void init()
    {
        adc_oneshot_unit_init_cfg_t unitCfg = {
            .unit_id = _unit,
            .clk_src = (adc_oneshot_clk_src_t)0,
        };
        ESP_ERROR_CHECK(adc_oneshot_new_unit(&unitCfg, &_handle));

        adc_oneshot_chan_cfg_t chanCfg = {
            .atten = _atten,
            .bitwidth = _bitwidth,
        };
        ESP_ERROR_CHECK(adc_oneshot_config_channel(_handle, _channel, &chanCfg));

        adc_cali_curve_fitting_config_t caliCfg = {
            .unit_id = _unit,
            .chan = _channel,
            .atten = _atten,
            .bitwidth = _bitwidth,
        };
        ESP_ERROR_CHECK(adc_cali_create_scheme_curve_fitting(&caliCfg, &_cali));
        ESP_LOGI("ADC", "init ok unit=%d ch=%d bits=%d", _unit, _channel, _maxRaw + 1);
    }

    int readRaw()
    {
        int raw = 0;
        ESP_ERROR_CHECK(adc_oneshot_read(_handle, _channel, &raw));
        return raw;
    }

    int toMv(int raw)
    {
        int mv = 0;
        ESP_ERROR_CHECK(adc_cali_raw_to_voltage(_cali, raw, &mv));
        return mv;
    }

    int computedMv(int raw)
    {
        return (raw * _maxMv) / _maxRaw;
    }

    float errorPct(int raw)
    {
        int cali = toMv(raw);
        return (cali > 0) ? ((float)(computedMv(raw) - cali) / cali * 100.0f) : 0.0f;
    }

    float percent(int raw)
    {
        return (raw * 100.0f) / _maxRaw;
    }

    float readPercent()
    {
        return percent(readRaw());
    }
};
