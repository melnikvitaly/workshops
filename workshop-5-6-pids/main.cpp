...
void pid_control_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (fan_pid.Ki > 0.0f)
        {
            ...
            if (fan_pid.integral > integral_limit)
                fan_pid.integral = integral_limit;
            if (fan_pid.integral < -integral_limit)
                fan_pid.integral = -integral_limit;
        }
        ...
        if (output > fan_pid.out_max)
            output = fan_pid.out_max;
        if (output < fan_pid.out_min)
            output = fan_pid.out_min;
