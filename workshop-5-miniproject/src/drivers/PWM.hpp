...
public:
    ...
    void init()
    {
        // Value-initialise, then assign. Designated initialisers would be
        // tidier, but ESP-IDF keeps adding members to these structs (sleep_mode,
        // deconfigure, flags...) and every addition breaks the build under
        // -Werror=missing-field-initializers. Zeroing first is stable across
        // IDF versions and gives the new members their intended defaults.
        ledc_timer_config_t timerCfg = {};
        timerCfg.speed_mode      = LEDC_LOW_SPEED_MODE;
        timerCfg.duty_resolution = _resolution;
        timerCfg.timer_num       = _timer;
        timerCfg.freq_hz         = _freqHz;
        timerCfg.clk_cfg         = LEDC_AUTO_CLK;
        ledc_timer_config(&timerCfg);

        ledc_channel_config_t channelCfg = {};
        channelCfg.gpio_num   = (int)_pin;
