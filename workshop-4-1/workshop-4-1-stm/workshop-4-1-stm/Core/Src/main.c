...
static void Blink_Startup_LED(void);
/* USER CODE BEGIN PFP */
...
int main(void)
{
  ...
  while (1)
  {
    /* Periodically transmit LED_ON / LED_OFF over UART and drive the LED */
    if (HAL_GetTick() - last_send >= SEND_PERIOD_MS) {
        last_send = HAL_GetTick();
        if (send_state == 0) {
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_RESET);
            strcpy(tx_buf, "LED_ON\n");
        } else {
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);
            strcpy(tx_buf, "LED_OFF\n");
        }
        send_state ^= 1;
        HAL_UART_Transmit(&huart1, (uint8_t*)tx_buf, strlen(tx_buf), 100);
    }

    if (HAL_UART_Receive(&huart1, &byte_in, 1, 100) == HAL_OK) {
        if (byte_in == '\n' || idx >= 31) {
            rx_buf[idx] = '\0';
            idx = 0;
            ...
            if (strcmp(rx_buf, "LED_ON") == 0) {
                HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_RESET);
                strcpy(tx_buf, "ACK_ON\n");
                HAL_UART_Transmit(&huart1, (uint8_t*)tx_buf, strlen(tx_buf), 100);
            }
            else if (strcmp(rx_buf, "LED_OFF") == 0) {
                HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);
                strcpy(tx_buf, "ACK_OFF\n");
                HAL_UART_Transmit(&huart1, (uint8_t*)tx_buf, strlen(tx_buf), 100);
            }
        } else {
            rx_buf[idx++] = byte_in;
