...
public:
    ...
    {
        ESP_LOGI(_tag, "\n%-5s | %-5s | %-11s | %-4s | %s",
                 "ADC", "SMA", "[min..max]", "LED%", "MOT%");
        ESP_LOGI(_tag, "-------|-------|-------------|------|-----");
    }

    void log(int adcRaw, int smaRaw, int rangeMin, int rangeMax,
             uint8_t ledPct, uint8_t motorPct)
    {
        if (!changed(_lastRaw, adcRaw) && !changed(_lastFilt, smaRaw)) return;
        _lastRaw  = adcRaw;
        _lastFilt = smaRaw;
        ESP_LOGI(_tag, "%5d | %5d | [%4d..%4d] | %3d%% | %3d%%",
                 adcRaw, smaRaw, rangeMin, rangeMax, ledPct, motorPct);
    }
};
