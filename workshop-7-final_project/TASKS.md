...
listed scores **39 / 42 (92.9%)**; the bar is 34 / 42 (80%).
...
- [ ] Timestamps owned by `AIM`: monotonic always, wall clock when available
...
- [ ] `AIM` board: ESP32-S3-WROOM-1, USB-C + BQ24040 + TLV758P (from `workshop-5-1`), servo rail, MOSFET laser driver, micro-SD, OLED header, UART1 header, E-stop + control buttons ⟦7.1⟧
...
### 11. Demo video ⟦9.1 9.2 9.3 9.4⟧ — *R-25 not yet accepted*
...
- [ ] `PILOT` input channel added to the exclusive selector
...
- [ ] `firmware/vault/` — STM32F401, SPI slave to `AIM`, DMA, double-buffered
- [ ] No parsing, no timestamps, no state — writes fully-formed records verbatim
- [ ] SPI master to the SD card; `F-22` rotation logic ported across unchanged
...
- [ ] **Q-8** — wall-clock source: SNTP, DS3231, or set by `EYE`?
- [ ] **Q-9** — local override for `input.channel` when the broker is down
- [ ] **Q-11** — log retention: segment size × count
- [ ] **Q-5** — parts on hand: joystick, SD module, ESP32-C3, servos
- [ ] **R-25** — accept the demo shot list (§11 above)
