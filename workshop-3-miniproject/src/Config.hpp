...
#define INPUT_SOURCE   INPUT_AUTO
...
namespace config
{
    ...
    // --- View port (working area) -------------------------------------------
    // Rectangle in the gimbal's normalised (x, y) space that every input maps
    // its output into. Full travel is [-1, 1] per axis; shrink these to limit
    // the working area (e.g. keep an auto sweep clear of the mechanical stops).
    constexpr float VIEW_X_MIN = -0.03f;
    constexpr float VIEW_X_MAX =  0.50f;
    constexpr float VIEW_Y_MIN = 0.05f;
    constexpr float VIEW_Y_MAX =  0.6f;
    ...
    constexpr uint32_t BEEP_FIRE_MS   = 40;
